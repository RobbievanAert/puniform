...
print.puniformoutput <- function(x) {
  ...
  if (x$method == "LNP" | x$method == "LN1MINP" | x$method == "P") {
    print(format(data.frame(est = round(x$est, 4), ci.lb = round(x$ci.lb, 4), ci.ub = round(x$ci.ub, 4), L.0 = round(x$L.0, 4), pval = round(x$pval.0, 4), ksig = x$ksig, row.names = ""), width = 9))
    cat("\n")
  }
